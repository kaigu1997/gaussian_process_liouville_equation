...
//    Uses the GSL and Eigen libraries for minimization and matrix operations 
//
...
class Gpr {
	public:
	    ...
	    void analyze();
	    double class_f (const gsl_vector *v, void* params);
		void setVector(gsl_vector* fv);
		void getVector(gsl_vector* gv);
		
		void maximizeHyperParameters();
		double testFunct(std::vector<double> &x);


	    
	private:
		int numP; // number of training points
		...
		std::vector<double> hyperLengthSq;
