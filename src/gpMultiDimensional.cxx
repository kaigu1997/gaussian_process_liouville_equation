...
int main(int argc,char** argv){
	...
	//
	std::ofstream dataFile("fit.dat");
	
	//libgp::GaussianProcess gp(2, "CovSum(CovSEiso, CovNoise)");
	libgp::GaussianProcess gp(2, "CovSum(CovSEard, CovNoise)");
   
    Eigen::VectorXd params(gp.covf().get_param_dim());
    //params << 0.10, 0.1, -0.01;
    cout << " must input " << params.size() << " parameters." << endl;
	for (int i=0;i<params.size()-1;i++) {
		params[i] = 0.1;
	}
	params[params.size()-1] = -0.01;
	
    gp.covf().set_loghyper(params);
    
    for (int i=0;i<numP;i++){
		double xv[Dimen];
		xv[0] = trainingPoints[i][0];
		xv[1] = trainingPoints[i][1];
		double y = funct( trainingPoints[i] );
		gp.add_pattern(xv,y);
	}
	...
	for (int ix=0;ix<=plot_points;ix++){
		for (int iy=0;iy <= plot_points;iy++){
			...
			double y_predicted_lib = gp.f(x);
			dataFile << xv[0] << " " << xv[1] << " " << yv << " " << y_predicted << " " << y_predicted_lib << std::endl;
		}
	}
	return 1;
}
