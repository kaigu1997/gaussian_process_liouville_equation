...
static QuantumDoubleMatrix diabatic_potential(const ClassicalDoubleVector& x)
{
	QuantumDoubleMatrix Potential;
	switch (TestModel)
	{
...
static Tensor3d diabatic_force(const ClassicalDoubleVector& x)
{
	Tensor3d Force;
	switch (TestModel)
	{
...
static QuantumDoubleMatrix diabatic_to_adiabatic_matrix(const ClassicalDoubleVector& x)
{
	const Eigen::SelfAdjointEigenSolver<QuantumDoubleMatrix> solver(diabatic_potential(x));
...
Tensor3d adiabatic_coupling(const ClassicalDoubleVector& x)
{
	...
	for (int i = 0; i < Dim; i++)
	{
		NAC[i] = QuantumDoubleMatrix::Zero();
...
/// and is the transformation matrix at a certain position \vec{x}.
static QuantumDoubleMatrix adiabatic_to_force_basis_matrix(const ClassicalDoubleVector& x, const int idx)
{
	const Eigen::SelfAdjointEigenSolver<QuantumDoubleMatrix> solver(adiabatic_force(x)[idx]);
	return solver.eigenvectors();
}

/// @see diabatic_force(), adiabatic_force()
///
/// Calculate by diagonalize the first element of the diabatic force. On other directions, F(force)=C^T*F(dia)^C, C the transformation matrix
...
QuantumComplexMatrix basis_transform(
	...
	const int idx_to)
{
	QuantumComplexMatrix result = rho;
	if (idx_from != idx_to)
	{
		...
		if (idx_from != Representation::Adiabatic)
		{
			...
			const QuantumComplexMatrix TransformMatrix = adiabatic_to_force_basis_matrix(x, idx_from).cast<std::complex<double>>();
		...
		if (idx_to != Representation::Adiabatic)
		{
			...
			const QuantumComplexMatrix TransformMatrix = adiabatic_to_force_basis_matrix(x, idx_to).cast<std::complex<double>>();
